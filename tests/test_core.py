...
def get_test_path(lab_no, task_no, test_no):
    return path.join(pkg_path, f"tests/test{lab_no}_tests/task{task_no}/test_{lab_no}_{task_no}_{test_no}")
...
class TestCore:
    ...
    def test(self, lab_no, task_no, test_func, func, *args):
        ...
        if lab_no == 1:
            ...
            limit = len(listdir(path.join(pkg_path, f"tests/test{lab_no}_tests/task{task_no}"))) // 2 + 1
